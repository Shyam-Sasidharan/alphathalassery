...
@section('content')
    ...
    <section class="content">
        ...
        <div class="row">
            <div class="col-md-12">
                <!-- form start -->
                <form role="form" action="{{ route('book.create') }}" lpformnum="1" method="post" enctype="multipart/form-data">
                    {!! csrf_field() !!}
                    <div class="box box-success">
                        <div class="box-header with-border">
                            <h3 class="box-title">Add Library Book</h3>
                                ...
                                <div class="form-group {{ $errors->first('library_id') ? 'has-error' : '' }}">
                                    ...
                                    <select name="library_id" id="library_id" class="form-control select2">
                                        @foreach(\App\Models\Library::orderBy('created_at')->get() as $category)
                                            <option {{old('library_id') == $category->id ? 'selected' : ''}} value="{{$category->id}}">{{$category->name}}</option>
                                        @endforeach
                                    </select>
